...
def get_intensity_satellite_frame(
    ...
    horizon_angle = np.arccos(lumos.constants.EARTH_RADIUS /
    ...
    for surface in sat_surfaces:
        ...
        surface_normalization = surface_normal[1] * \
            vector_2_sun[1] + surface_normal[2] * vector_2_sun[2]
        surface_normalization = np.clip(surface_normalization, 0, None)

        observer_normalization = surface_normal[0] * sat_obs_x + \
            surface_normal[1] * sat_obs_y + surface_normal[2] * sat_obs_z
        ...
        intensity = intensity + lumos.constants.SUN_INTENSITY * surface.area * \
            (sun_contribution + earth_contribution) / dist_sat_2_obs ** 2

    return intensity

##### Un-comment to use ceres calculated magnitudes #######
    #         earth_contribution = earth_contribution \
    #             + np.sum(panel_brdfs * surface_brdf
    #                      * panel_normalizations * observer_normalization
    #                      * surface_normalizations * panel_observing_normalization
    #                      * panel_areas / dist_panels_2_sat**2)
    #
    #     sun_intensity = sun_intensity + lumos.constants.SUN_INTENSITY * surface.area * \
    #         (sun_contribution) / dist_sat_2_obs ** 2
    #
    #     earth_intensity = earth_intensity + lumos.constants.EARTHSHINE_AVERAGE * surface.area * \
    #         (earth_contribution) / dist_sat_2_obs ** 2
    #
    #     # print("Earth" + str(earth_intensity))
    #     # print("Sun" + str(sun_intensity))
    #
    # return sun_intensity + earth_intensity
...
def get_brightness_coords(
    ...
    phi = np.arccos(sat_z) - np.arcsin(lumos.constants.EARTH_RADIUS /
                                       (lumos.constants.EARTH_RADIUS + sat_height) * np.sqrt(sat_x**2 + sat_y**2))
    ...
    obs_z = (lumos.constants.EARTH_RADIUS + sat_height) - \
        dist_to_sat * (T31 * sat_x + T32 * sat_y + T33 * sat_z)
